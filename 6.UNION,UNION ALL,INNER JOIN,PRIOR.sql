...
-- ## UNION
-- 1. 합집합 연산의 의미입니다.
-- 2. 첫번째 쿼리와 두번째 쿼리의 중복정보는 한번만 보여줍니다 (중복제거!).
...
-- 2. UNION과는 달리 중복된 데이터 ! ! 도 한번 더 ! ! 보여줍니다.
-- 3. 자동 정렬 기능을 지원하지 않아 성능상 유리합니다.
-- 4 . SELECT ENAME 두개 있을때 위에꺼 기준으로 정의 !
...
-- 계층형 쿼리 
-- START WITH : 계층의 첫 단계를 어디서 시작할 것인지의 대한 조건 ,  이 조건에 대한 거를 수 없다
...
--  prior 위치에 따라 순방향인지 역바항 인지 알 수 있다.
--     PRIOR 위치 부모 (prior) / 자식  => 순방향 전개
--     부모 / 자식 (prior) => 역방향
