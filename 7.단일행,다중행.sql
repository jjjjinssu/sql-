...
SELECT 
emp_no,emp_nm,salary,
FROM TB_EMP te 
JOIN
ON 

--JOIN  ON : 두개이상 테이블에서 지정된 조건에 따라 데이터를 결합하여 하나의 결과 집합으로 표시하는데 사용
--JOIN 조건은 ON 키워드를 사용하여 지정되며 , 두 테이블을 일치시키는데 사용된다.
SELECT 
...
--            250 ->  200, 300, 400 중에 없으므로 false , 포함되어야 함 ! !
...
-- 4. EXISTS : 메인쿼리의 비교조건이 서브쿼리의 결과 중 만족하는 값! 이 하나 ! 라도 존재하면 참


-- 5. NOT EXISTS 매칭 X

-- EXISTS / NOT EXISTS 는   SELECT 볼 필요 없고 , 안에있는 서브쿼리 WHERE 절로 파악 !!!!!!!!!!!!!!
