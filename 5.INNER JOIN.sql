...
-- CROSS JOIN(가로 로 연결), 카테시안 곱 (cartesian product)
-- JOIN 특징 가로 로 연결    2 + 3 
--  결과의 행수는 곱으로 들어감  3 * 4
-- 따라서, Inner join은 두 테이블 사이에서 일치하는 값을 찾아서 출력하는 반면,
-- cross join은 두 테이블 간의 모든 가능한 조합을 출력합니다.]
...
SELECT 
	...
	D.DEPT_CD,
	D.DEPT_NM 
FROM tb_emp E,tb_deptD
WHERE tb_emp.DEPT_CD = tb_dept.DEPT_CD 
;


-- INNER JOIN (EQUI JOIN의 한 종류) 같은 컬럼이 있어야함
SELECT 
    *
FROM test_a, test_b
...
WHERE A.certi_cd = B.certi_cd              --두개 조인 하면 조건 1개
...
WHERE A.certi_cd = B.certi_cd            -- 세개 조인 하면 조건 2개
...
SELECT 
    A.emp_no, A.emp_nm, A.addr, A.dept_cd, B.dept_nm
FROM tb_emp A
INNER JOIN tb_dept B       --  INNER JOIN은 매칭 되는거만 가져옴
...
FROM tb_emp E 
JOIN tb_dept D                  -- 하나하나 조인 할 수 있다.
ON E.dept_cd = D.dept_cd
JOIN tb_emp_certi EC 
ON E.emp_no = EC.emp_no
JOIN tb_certi C
ON EC.certi_cd = C.certi_cd
WHERE E.birth_de BETWEEN '19800101' AND '19891231'  --일반조건
;
...
SELECT 
    A.emp_no, A.emp_nm, dept_cd, B.dept_nm        -- B.dept_cd (x)
FROM tb_emp A
...
--    조인이 일어나지만 USING을 사용하면 원하는 컬럼에 대해서면 
--	         선택적!!!
--   		 조인조건 !!  
--    부여할 수 있습니다.
